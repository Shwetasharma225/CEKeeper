...
const ThModal = ({ RegistrationId, LicenceId, FinId }) => {
  ...
  const handleDownload = async (path) => {
    try {
      ...
      link.setAttribute('download', url.split('/').pop()); // Extracts the filename from the URL
  ...
  const handleView = (path) => {
    ...
    if (isImage) {
      ...
      newWindow.document.write(`<img src="${url}" style="max-width:100%; max-height:100%;" />`);
    } else {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const response = await axios.post('http://warals1.ddns.net:8045/api/TotalHours', {
